package f.e.a.j.b0;

import com.comeback.data.ui.lutube.LutubeLaunchActivity;
import i.h0;
import java.io.IOException;

/* compiled from: LutubeLaunchActivity.java */
/* loaded from: classes.dex */
public class h implements i.g {
    public boolean a = false;
    public final /* synthetic */ LutubeLaunchActivity b;

    public h(LutubeLaunchActivity lutubeLaunchActivity) {
        this.b = lutubeLaunchActivity;
    }

    @Override // i.g
    public void c(i.f fVar, h0 h0Var) throws IOException {
        String replace = fVar.S().b.f6854j.replace(f.e.a.e.a("GBETAQ4XF1sHC1g="), "");
        if (h0Var.f6524e > 300) {
            f.e.a.e.a("RBYRAQoeAw==");
            f.e.a.e.a("0uXZgNH1CgND");
            f.e.a.k.f.a();
        } else if (this.a) {
        } else {
            this.a = true;
            f.e.a.k.b.s = replace;
            LutubeLaunchActivity.m(this.b);
            LutubeLaunchActivity.n(this.b);
            f.e.a.e.a("RBYRAQoeAw==");
            f.e.a.e.a("0erzgeHs");
            f.e.a.k.f.a();
        }
    }

    @Override // i.g
    public void d(i.f fVar, IOException iOException) {
        f.e.a.e.a("RBYRAQoeAw==");
        fVar.S().b.f6854j.replace(f.e.a.e.a("GBETAQ4XF1sHC1g="), "");
        f.e.a.e.a("0sbSjN/W");
        f.e.a.k.f.a();
    }
}
