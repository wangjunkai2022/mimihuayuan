...
public class b {
    ...
    public static String e(Context context) {
        if (TextUtils.isEmpty(f2692d)) {
            if (Build.VERSION.SDK_INT < 23) {
                try {
                    WifiManager wifiManager = (WifiManager) context.getApplicationContext().getSystemService("wifi");
                    WifiInfo connectionInfo = wifiManager == null ? null : wifiManager.getConnectionInfo();
                    f2692d = connectionInfo == null ? "" : connectionInfo.getMacAddress();
                } catch (Exception e2) {
                    e2.printStackTrace();
                }
            } else {
    ...
    public static String f(Context context) {
        if (!TextUtils.isEmpty(f2693e)) {
            return f2693e;
        }
        try {
            f2693e = Settings.Secure.getString(context.getContentResolver(), com.umeng.commonsdk.statistics.idtracking.b.a);
        } catch (Exception e2) {
            e2.printStackTrace();
        }
        return f2693e;
