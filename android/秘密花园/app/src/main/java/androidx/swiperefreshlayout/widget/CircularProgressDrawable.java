...
public class CircularProgressDrawable extends Drawable implements Animatable {
    ...
    private void applyFinishTranslation(float f2, Ring ring) {
        ...
        ring.setStartTrim((((ring.getStartingEndTrim() - 0.01f) - ring.getStartingStartTrim()) * f2) + ring.getStartingStartTrim());
        ring.setEndTrim(ring.getStartingEndTrim());
        ring.setRotation(((((float) (Math.floor((double) (ring.getStartingRotation() / 0.8f)) + 1.0d)) - ring.getStartingRotation()) * f2) + ring.getStartingRotation());
    }
    ...
    public void applyTransformation(float f2, Ring ring, boolean z) {
        ...
        if (this.mFinishing) {
            ...
            if (f2 < 0.5f) {
                ...
                f3 = startingStartTrim - (((1.0f - MATERIAL_INTERPOLATOR.getInterpolation((f2 - 0.5f) / 0.5f)) * 0.79f) + 0.01f);
                f4 = startingStartTrim;
            }
            ring.setStartTrim(f3);
            ring.setEndTrim(f4);
            ring.setRotation((0.20999998f * f2) + startingRotation);
