...
public abstract class AutoScrollHelper implements View.OnTouchListener {
    ...
    public static class ClampedScroller {
        ...
        private float getValueAt(long j2) {
            ...
            if (j3 < 0 || j2 < j3) {
                return AutoScrollHelper.constrain(((float) (j2 - this.mStartTime)) / ((float) this.mRampUpDuration), 0.0f, 1.0f) * 0.5f;
            }
            long j4 = j2 - j3;
            float f2 = this.mStopValue;
            return (AutoScrollHelper.constrain(((float) j4) / ((float) this.mEffectiveRampDown), 0.0f, 1.0f) * f2) + (1.0f - f2);
        }
