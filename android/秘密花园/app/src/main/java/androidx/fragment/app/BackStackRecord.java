...
public final class BackStackRecord extends FragmentTransaction implements FragmentManager.BackStackEntry, FragmentManagerImpl.OpGenerator {
    ...
    public FragmentTransaction setMaxLifecycle(@NonNull Fragment fragment, @NonNull Lifecycle.State state) {
        if (fragment.mFragmentManager != this.mManager) {
            StringBuilder o = a.o("Cannot setMaxLifecycle for Fragment not attached to FragmentManager ");
            o.append(this.mManager);
            throw new IllegalArgumentException(o.toString());
        } else if (state.isAtLeast(Lifecycle.State.CREATED)) {
            return super.setMaxLifecycle(fragment, state);
        } else {
            StringBuilder o2 = a.o("Cannot set maximum Lifecycle below ");
            o2.append(Lifecycle.State.CREATED);
