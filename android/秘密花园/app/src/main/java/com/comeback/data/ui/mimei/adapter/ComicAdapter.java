package com.comeback.data.ui.mimei.adapter;

import android.content.Context;
import android.view.View;
import android.view.ViewGroup;
import android.widget.TextView;
import androidx.annotation.CallSuper;
import androidx.annotation.NonNull;
import androidx.annotation.UiThread;
import androidx.recyclerview.widget.RecyclerView;
import butterknife.BindView;
import butterknife.Unbinder;
import c.a.a.b.g.h;
import com.comeback.data.base.BaseAbstractAdapter;
import com.comeback.data.base.BaseViewHolder;
import com.comeback.data.ui.mimei.ComicReadActivity;
import com.comeback.data.ui.mimei.bean.MimeiComicItem;
import com.comeback.data.widget.RatioImageView;
import com.comeback.secret.garden.R;
import d.c.b;
import d.c.c;
import f.e.a.e;

/* loaded from: classes.dex */
public class ComicAdapter extends BaseAbstractAdapter<MimeiComicItem> {

    /* loaded from: classes.dex */
    public static class Holder extends BaseViewHolder<MimeiComicItem> {
        public ComicAdapter a;
        public MimeiComicItem b;
        @BindView
        public RatioImageView ivCover;
        @BindView
        public TextView tvName;
        @BindView
        public TextView tvState;
        @BindView
        public TextView tvState2;

        public Holder(View view, ComicAdapter comicAdapter) {
            super(view);
            this.a = comicAdapter;
        }

        /* JADX DEBUG: Method arguments types fixed to match base method, original types: [java.lang.Object] */
        @Override // com.comeback.data.base.BaseViewHolder
        public void a(MimeiComicItem mimeiComicItem) {
            MimeiComicItem mimeiComicItem2 = mimeiComicItem;
            this.b = mimeiComicItem2;
            this.tvName.setText(mimeiComicItem2.getName());
            h.J0(mimeiComicItem2.getFull_cover_path(), this.ivCover);
            this.tvState2.setVisibility(mimeiComicItem2.isEnd() ? 8 : 0);
            this.tvState.setText(mimeiComicItem2.getChapterState());
        }
    }

    /* loaded from: classes.dex */
    public class Holder_ViewBinding implements Unbinder {
        public Holder b;

        /* renamed from: c  reason: collision with root package name */
        public View f1688c;

        /* compiled from: ComicAdapter$Holder_ViewBinding.java */
        /* loaded from: classes.dex */
        public class a extends b {

            /* renamed from: c  reason: collision with root package name */
            public final /* synthetic */ Holder f1689c;

            public a(Holder_ViewBinding holder_ViewBinding, Holder holder) {
                this.f1689c = holder;
            }

            @Override // d.c.b
            public void a(View view) {
                Holder holder = this.f1689c;
                ComicReadActivity.q(holder.a.a, holder.b);
            }
        }

        @UiThread
        public Holder_ViewBinding(Holder holder, View view) {
            this.b = holder;
            holder.ivCover = (RatioImageView) c.c(view, R.id.iv_cover, e.a("UQsGCA9THloFJVsRHRhE"), RatioImageView.class);
            holder.tvName = (TextView) c.c(view, R.id.tv_name, e.a("UQsGCA9THkcFKFUKHU0="), TextView.class);
            holder.tvState = (TextView) c.c(view, R.id.tv_state, e.a("UQsGCA9THkcFNUAGDA9E"), TextView.class);
            holder.tvState2 = (TextView) c.c(view, R.id.tv_state2, e.a("UQsGCA9THkcFNUAGDA9RTA=="), TextView.class);
            View b = c.b(view, R.id.rl_root, e.a("WgcXDAQXGRQEB0AEEE0="));
            this.f1688c = b;
            b.setOnClickListener(new a(this, holder));
        }

        @Override // butterknife.Unbinder
        @CallSuper
        public void a() {
            Holder holder = this.b;
            if (holder != null) {
                this.b = null;
                holder.ivCover = null;
                holder.tvName = null;
                holder.tvState = null;
                holder.tvState2 = null;
                this.f1688c.setOnClickListener(null);
                this.f1688c = null;
                return;
            }
            throw new IllegalStateException(e.a("dQsNAAIdXkBTB1gVHQsHEhcBDwEKAVxXXQ=="));
        }
    }

    public ComicAdapter(Context context) {
        super(context);
    }

    @NonNull
    public Holder f(@NonNull ViewGroup viewGroup) {
        return new Holder(this.f107c.inflate(R.layout.item_mimei_comic2, viewGroup, false), this);
    }

    @Override // androidx.recyclerview.widget.RecyclerView.Adapter
    @NonNull
    public /* bridge */ /* synthetic */ RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i2) {
        return f(viewGroup);
    }
}
