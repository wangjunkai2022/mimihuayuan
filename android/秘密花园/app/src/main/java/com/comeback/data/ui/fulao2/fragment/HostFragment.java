package com.comeback.data.ui.fulao2.fragment;

import android.text.TextUtils;
import android.view.View;
import android.widget.TextView;
import androidx.appcompat.app.AlertDialog;
import butterknife.BindView;
import butterknife.OnClick;
import com.comeback.data.base.BaseFragment;
import com.comeback.data.ui.fulao2.bean.Host;
import com.comeback.data.ui.fulao2.bean.VideoInfo;
import com.comeback.secret.garden.R;
import f.e.a.e;
import f.e.a.j.k.g.a;
import f.e.a.j.k.g.c;
import f.e.a.j.k.g.d;
import f.e.a.j.k.h.f;
import f.e.a.k.b;
import f.e.a.k.j;

/* loaded from: classes.dex */
public class HostFragment extends BaseFragment {

    /* renamed from: h  reason: collision with root package name */
    public static String f750h = e.a("UQ5ROwgcV1UaAQ==");

    /* renamed from: i  reason: collision with root package name */
    public static String f751i = e.a("UQ5ROwgcV1UaAWsOFQsEDg==");

    /* renamed from: j  reason: collision with root package name */
    public static String f752j = e.a("UQ5ROwgcV1UaAWsUDBgGClpQ");

    /* renamed from: k  reason: collision with root package name */
    public static String f753k = e.a("UQ5ROwgcV1UaAWsPFxkX");

    /* renamed from: g  reason: collision with root package name */
    public Host f754g;
    @BindView
    public TextView tvHost;
    @BindView
    public TextView tvImage;
    @BindView
    public TextView tvStream;

    @Override // com.comeback.data.base.BaseFragment
    public int b() {
        return R.layout.fragment_fulao2_host;
    }

    @Override // com.comeback.data.base.BaseFragment
    public void e() {
    }

    @Override // com.comeback.data.base.BaseFragment
    public void f() {
        String d2 = j.b().d(f750h);
        if (TextUtils.isEmpty(d2)) {
            d2 = e.a("");
        }
        this.f754g = (Host) new f.i.b.j().d(d2, Host.class);
        this.tvHost.setText(j.b().e(f753k, b.y));
        this.tvImage.setText(j.b().e(f751i, b.z));
        this.tvStream.setText(j.b().e(f752j, VideoInfo.StreamHost));
    }

    @OnClick
    public void onViewClicked(View view) {
        m.e<Host> eVar;
        switch (view.getId()) {
            case R.id.tv_host_change /* 2131296960 */:
                new AlertDialog.Builder(getContext()).setSingleChoiceItems(this.f754g.getApiString(), -1, new a(this)).show();
                return;
            case R.id.tv_image_change /* 2131296964 */:
                new AlertDialog.Builder(getContext()).setSingleChoiceItems(this.f754g.getImageString(), -1, new f.e.a.j.k.g.b(this)).show();
                return;
            case R.id.tv_refresh /* 2131297018 */:
                long currentTimeMillis = System.currentTimeMillis() % 3;
                if (currentTimeMillis == 0) {
                    eVar = f.b().b();
                } else if (currentTimeMillis == 1) {
                    eVar = f.b().m();
                } else {
                    eVar = f.b().k();
                }
                g(eVar, new d(this));
                return;
            case R.id.tv_stream_change /* 2131297044 */:
                new AlertDialog.Builder(getContext()).setSingleChoiceItems(this.f754g.getStreamString(), -1, new c(this)).show();
                return;
            default:
                return;
        }
    }
}
