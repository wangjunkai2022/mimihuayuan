...
public class BallZigZagIndicator extends Indicator {
    ...
    public ArrayList<ValueAnimator> onCreateAnimators() {
        ...
        for (final int i2 = 0; i2 < 2; i2++) {
            ValueAnimator ofFloat = ValueAnimator.ofFloat(width, ((float) getWidth()) - width, (float) (getWidth() / 2), width);
            if (i2 == 1) {
                ofFloat = ValueAnimator.ofFloat(((float) getWidth()) - width, width, (float) (getWidth() / 2), ((float) getWidth()) - width);
            }
            ValueAnimator ofFloat2 = ValueAnimator.ofFloat(width2, width2, (float) (getHeight() / 2), width2);
            if (i2 == 1) {
                ofFloat2 = ValueAnimator.ofFloat(((float) getHeight()) - width2, ((float) getHeight()) - width2, (float) (getHeight() / 2), ((float) getHeight()) - width2);
            }
