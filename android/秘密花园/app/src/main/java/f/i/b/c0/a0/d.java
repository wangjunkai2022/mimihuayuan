package f.i.b.c0.a0;

import f.i.b.a0;
import f.i.b.c0.g;
import f.i.b.d0.a;
import f.i.b.j;
import f.i.b.n;
import f.i.b.v;
import f.i.b.y;
import f.i.b.z;

/* compiled from: JsonAdapterAnnotationTypeAdapterFactory.java */
/* loaded from: classes.dex */
public final class d implements a0 {
    public final g a;

    public d(g gVar) {
        this.a = gVar;
    }

    @Override // f.i.b.a0
    public <T> z<T> a(j jVar, a<T> aVar) {
        f.i.b.b0.a aVar2 = (f.i.b.b0.a) aVar.a.getAnnotation(f.i.b.b0.a.class);
        if (aVar2 == null) {
            return null;
        }
        return (z<T>) b(this.a, jVar, aVar, aVar2);
    }

    public z<?> b(g gVar, j jVar, a<?> aVar, f.i.b.b0.a aVar2) {
        z<?> zVar;
        Object a = gVar.a(new a(aVar2.value())).a();
        if (a instanceof z) {
            zVar = (z) a;
        } else if (a instanceof a0) {
            zVar = ((a0) a).a(jVar, aVar);
        } else {
            boolean z = a instanceof v;
            if (z || (a instanceof n)) {
                n nVar = null;
                v vVar = z ? (v) a : null;
                if (a instanceof n) {
                    nVar = (n) a;
                }
                zVar = new m<>(vVar, nVar, jVar, aVar, null);
            } else {
                StringBuilder o = f.b.a.a.a.o("Invalid attempt to bind an instance of ");
                o.append(a.getClass().getName());
                o.append(" as a @JsonAdapter for ");
                o.append(aVar.toString());
                o.append(". @JsonAdapter value must be a TypeAdapter, TypeAdapterFactory, JsonSerializer or JsonDeserializer.");
                throw new IllegalArgumentException(o.toString());
            }
        }
        return (zVar == null || !aVar2.nullSafe()) ? zVar : new y(zVar);
    }
}
