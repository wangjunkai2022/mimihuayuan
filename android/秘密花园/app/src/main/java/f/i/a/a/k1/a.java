...
public class a extends View implements g {
    ...
    public final Rect seekBounds = new Rect();
    public final Rect progressBar = new Rect();
    public final Rect bufferedBar = new Rect();
    public final Rect scrubberBar = new Rect();
    public final Paint playedPaint = new Paint();
    public final Paint bufferedPaint = new Paint();
    public final Paint unplayedPaint = new Paint();
    public final Paint adMarkerPaint = new Paint();
    public final Paint playedAdMarkerPaint = new Paint();
    public final CopyOnWriteArraySet<g.a> listeners = new CopyOnWriteArraySet<>();
    ...
    public a(Context context, AttributeSet attributeSet) {
        ...
        this.stopScrubbingRunnable = new RunnableC0079a();
    ...
    private void drawPlayhead(Canvas canvas) {
        ...
        if (this.duration > 0) {
            ...
            if (drawable == null) {
                ...
                canvas.drawCircle((float) o, (float) centerY, (float) (i2 / 2), this.scrubberPaint);
                return;
            }
    ...
    private void drawTimeBar(Canvas canvas) {
        ...
        if (this.duration <= 0) {
            ...
            canvas.drawRect((float) rect.left, (float) centerY, (float) rect.right, (float) i2, this.unplayedPaint);
            return;
        }
        ...
        if (max < i5) {
            canvas.drawRect((float) max, (float) centerY, (float) i5, (float) i2, this.unplayedPaint);
        }
        int max2 = Math.max(i3, this.scrubberBar.right);
        if (i4 > max2) {
            canvas.drawRect((float) max2, (float) centerY, (float) i4, (float) i2, this.bufferedPaint);
        }
        if (this.scrubberBar.width() > 0) {
            Rect rect3 = this.scrubberBar;
            canvas.drawRect((float) rect3.left, (float) centerY, (float) rect3.right, (float) i2, this.playedPaint);
        }
        int i6 = this.adMarkerWidth / 2;
        for (int i7 = 0; i7 < this.adGroupCount; i7++) {
            long p = h0.p(this.adGroupTimesMs[i7], 0, this.duration);
            Rect rect4 = this.progressBar;
            int min = Math.min(rect4.width() - this.adMarkerWidth, Math.max(0, ((int) ((((long) this.progressBar.width()) * p) / this.duration)) - i6)) + rect4.left;
            canvas.drawRect((float) min, (float) centerY, (float) (min + this.adMarkerWidth), (float) i2, this.playedAdGroups[i7] ? this.playedAdMarkerPaint : this.adMarkerPaint);
        }
    }
    ...
    private boolean scrubIncrementally(long j2) {
        ...
        long p = h0.p(scrubberPosition + j2, 0, this.duration);
    ...
    private void update() {
        ...
        if (this.duration > 0) {
            ...
            int width2 = (int) ((((long) this.progressBar.width()) * j2) / this.duration);
            Rect rect3 = this.scrubberBar;
            Rect rect4 = this.progressBar;
