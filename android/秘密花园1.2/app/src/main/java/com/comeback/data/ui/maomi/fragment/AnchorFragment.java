...
public class AnchorFragment extends BaseFragment {
    ...
    public ArrayList<BaseFragment> f1686g;

    /* renamed from: h  reason: collision with root package name */
    public ViewPager f1687h;
    @BindView
    ...
    public static void h(AnchorFragment anchorFragment, AnchorList anchorList) {
        if (anchorFragment != null) {
            anchorFragment.f1686g = new ArrayList<>();
            for (AnchorList.DataBean dataBean : anchorList.getData()) {
                anchorFragment.f1686g.add(AnchorCardFragment.h(dataBean));
            }
            ...
            anchorFragment.f1687h = viewPager;
            viewPager.setOffscreenPageLimit(anchorFragment.f1686g.size() - 1);
            anchorFragment.f1687h.setAdapter(new BaseViewPagerAdapter(anchorFragment.getChildFragmentManager(), null, anchorFragment.f1686g));
            anchorFragment.f1687h.setPageTransformer(false, new BigScaleTransformer());
            anchorFragment.f1687h.setCurrentItem(0, false);
            return;
        }
        throw null;
    }
