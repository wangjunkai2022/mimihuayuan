...
public class QueryDescriptor {
    ...
    public static void checkFieldType(c cVar, Set<RealmFieldType> set, String str, String str2) {
        cVar.b();
        if (set.contains(cVar.f6336e)) {
            return;
        }
        Locale locale = Locale.US;
        cVar.b();
        cVar.b();
        throw new IllegalArgumentException(String.format(locale, "%s on '%s' field '%s' in '%s'.", str, cVar.f6336e, cVar.f6335d, str2));
    }
    ...
    public static QueryDescriptor getInstanceForDistinct(c.a aVar, Table table, String[] strArr) {
        return getInstance(aVar, table, strArr, null, c.f6333l, DISTINCT_VALID_FIELD_TYPES, "Distinct is not supported");
    }
    ...
    public static QueryDescriptor getInstanceForSort(c.a aVar, Table table, String[] strArr, m0[] m0VarArr) {
        if (m0VarArr != null && m0VarArr.length != 0) {
            if (strArr.length == m0VarArr.length) {
                return getInstance(aVar, table, strArr, m0VarArr, c.f6332k, SORT_VALID_FIELD_TYPES, "Sort is not supported");
            }
