...
public class BallSpinFadeLoaderIndicator extends Indicator {
    ...
    public Point circleAt(int i2, int i3, float f2, double d2) {
        double d3 = f2;
        return new Point((float) ((Math.cos(d2) * d3) + (i2 / 2)), (float) ((Math.sin(d2) * d3) + (i3 / 2)));
    }
    ...
    public void draw(Canvas canvas, Paint paint) {
        ...
        for (int i2 = 0; i2 < 8; i2++) {
            canvas.save();
            Point circleAt = circleAt(getWidth(), getHeight(), (getWidth() / 2) - width, 0.7853981633974483d * i2);
